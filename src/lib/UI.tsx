...
export function populateApiPayload({ type, val }: IpopulateSwPayload) {
	if (type === "VID") {
		const currMp4s = currentMp4Sources();
		if (!currMp4s || !currMp4s.length) return;

		const matching = currMp4s.find((v) => v.refId === val);
		...
		const videoSources = wholeBooksOptionsForSelect();
		if (!videoSources || !videoSources.length) return;

		setDownloadPreference((prev) => {
			return {
				...prev,
				apiPayload: videoSources,
			};
		});
	}
}
