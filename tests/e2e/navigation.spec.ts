...
test.describe('Navigation Tests', () => {
  ...
  test('navigate between books in the Bible', async ({ page }) => {
    ...
    for (const book of TEST_BOOKS) {
      ...
      await expect(page.getByTestId(`book-button-${book.toLowerCase()}`)).toHaveClass(/underline/);
  ...
  test('dynamic routing works correctly for book/chapter/verse patterns', async ({ page }) => {
    ...
    await expect(page.getByTestId('chapter-button-003')).toHaveClass(/scale-120/);
    ...
    await expect(page.getByTestId('chapter-button-002')).toHaveClass(/scale-120/);
  });
  ...
  test('browser back/forward navigation works', async ({ page }) => {
    ...
    // Go back
    await page.goBack();
    await expect(page).toHaveURL(/\/MAT\.5$/);

    // Go forward
    await page.goForward();
    await expect(page).toHaveURL(/\/MRK\.3$/);
  });
  ...
  test('chapter navigation updates correctly when switching books', async ({ page }) => {
